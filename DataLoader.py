...
class AOIDataset(Dataset) : 
    def __init__(self, train : bool =None, val : bool =None, test : bool =None, transform=None) : 
        ...
        if train : 
            data = pd.read_csv('/home/injo/workspaces/VAE/data/train.csv')
        elif val : 
            data = pd.read_csv('/home/injo/workspaces/VAE/data/valid.csv')
        else : 
            data = pd.read_csv('/home/injo/workspaces/VAE/data/test.csv')
...
class XrayDataset(Dataset) : 
    def __init__(self, train : bool =None, val : bool =None, test : bool =None, transform=None) : 
        """
        Xray 데이터를 불러오기 위한 데이터셋 클래스입니다. 
        작업 공간내에 준영님이 올려주신 train, validation,test csv 파일을 업로드해주시고, 아래의 read_csv의 파일 Path를 변경하여 사용해주시면 됩니다. 
        ...
        if train : 
            data = pd.read_csv('/home/injo/workspaces/resnet_evaluation/data/xray/train.csv')
        elif val : 
            data = pd.read_csv('/home/injo/workspaces/resnet_evaluation/data/xray/valid.csv')
        else : 
            data = pd.read_csv('/home/injo/workspaces/resnet_evaluation/data/xray/test.csv')
        
        self.imgs = data['path']
        self.imgs = list(self.imgs)
        self.labels = data['label']
        self.label_tags = {'good' : 0,
                            'bad' : 1,
                            'empty' : 2,
                            'in-spec' : 3}
